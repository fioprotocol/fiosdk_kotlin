...
for ARCH in $ARCHES; do
  bash -c "$SDK/cmake/3.6.4111459/bin/cmake -H$BASE -B$BASE/target/build/$ARCH -DCMAKE_TOOLCHAIN_FILE=$SDK/ndk-bundle/build/cmake/android.toolchain.cmake -DCMAKE_LIBRARY_OUTPUT_DIRECTORY=$BASE/target/binaries/$ARCH -DANDROID_NDK=$SDK/ndk-bundle -DANDROID_ABI=$ARCH -DCMAKE_MAKE_PROGRAM=$SDK/cmake/3.6.4111459/bin/ninja -DANDROID_NATIVE_API_LEVEL=21 -DANDROID_PLATFORM=android-21 -DCMAKE_BUILD_TYPE=Release -G'Android Gradle - Ninja'"
  R=$?
  if [[ $R != 0 ]]; then
